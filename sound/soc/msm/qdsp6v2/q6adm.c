...
{
	struct adm_cmd_set_pp_params	admp;
	...
	*ob_params++ = size;
	...
	admp.apr_hdr.opcode = ADM_CMD_SET_PP_PARAMS_V5;
	admp.mem_hdr.data_payload_addr_lsw = lower_32_bits(this_adm.outband_memmap.paddr);
	admp.mem_hdr.data_payload_addr_msw = msm_audio_populate_upper_32_bits(
						this_adm.outband_memmap.paddr);
	admp.mem_hdr.mem_map_handle = atomic_read(&this_adm.mem_map_handles[
					  ADM_DTS_EAGLE]);
	admp.payload_size = size + sizeof(struct param_hdr_v3);

	pr_debug("DTS_EAGLE_ADM: %s - Command was sent now check Q6 - port id = %d, size %d, module id %x, param id %x.\n",
			__func__, admp.apr_hdr.dest_port,
		      ...
		      void *data, uint32_t size)
{
	...
	*ob_params++ = size;
	...
	admp.mem_hdr.mem_map_handle = atomic_read(&this_adm.mem_map_handles[
					  ADM_DTS_EAGLE]);
	admp.param_hdr.v3.module_id = AUDPROC_MODULE_ID_DTS_HPX_POSTMIX;
	admp.param_hdr.v3.param_id = param_id;
	admp.param_hdr.v3.param_size = size + sizeof(struct param_hdr_v3);
	admp.param_hdr.v3.reserved = 0;
